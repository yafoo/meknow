/**
 * MeNote 集成 P2P 服务 —— lib/p2p.js
 *
 * 用官方 @number0/iroh 库把整个 MeNote Web 服务（jj.js/Koa 应用）通过
 * P2P QUIC 隧道暴露给手机端 App，无需公网 IP、端口映射。
 *
 * 架构：
 *   手机 App (iroh QUIC, ALPN menote-p2p/1)
 ...
 *   2. MeNote 原有登录认证：手机端仍需登录 admin 后台。
 *
 * 配对流程：
...
const P2P = {
    ...
    listenServer: null,      // 主服务监听句柄（server.js 传入；退出时先解绑端口）
     ...
     * @param {object} [options] {listenServer: 主服务监听句柄（server.js 传入，
     *   优雅退出时第一时间解绑端口）；dataDir: 测试隔离目录}
     */
    async init(app, options = {}) {
        if(this.running) return;
        this.app = app;
        this.listenServer = options.listenServer || null;
        ...
        Logger.system('[p2p] MeNote P2P 服务已启动');
        ...
        // 优雅退出：第一次 Ctrl+C 优雅关闭，完成后主动 exit ——
        // 否则 Node 默认退出行为被 once listener 拦截，进程挂着要按两次 Ctrl+C
        const shutdown = async (signal) => {
            // 3s 兜底：endpoint.close() 卡住时也保证退出（避免优雅关闭变成挂死）
    ...
    async handleIncoming(incoming) {
        ...
        // 同节点重连（App 重启/网络切换）：主动关闭旧连接 ——
        // ① 旧连接可能已死但 QUIC 未感知，占用 onlinePeers 槽位
        // ② 防止 removePeer 时踢到已废弃的旧连接（新连接不断线的 bug 根因）
        const prevOnline = this.onlinePeers.get(peerId);
        if(prevOnline?.conn && prevOnline.conn !== conn) {
            Logger.system(`[p2p] ⇄ 节点 ${name || peerId.substring(0, 10)}… 重新连接，关闭旧连接`);
            try { prevOnline.conn.close(1000n, Array.from(Buffer.from('superseded'))); } catch(_) {}
        }

        // 立即登记连接引用（踢线/状态查询用）——不等 trackPath 的路径就绪，
        // 否则新连接建立瞬间 paths() 为空时 onlinePeers 里残留旧 conn
        this.onlinePeers.set(peerId, {path: '', addr: '', conn});
        ...
        const trackPath = () => {
            try {
                ...
                if(selected) {
                    ...
                    if(prev && prev.path && prev.path !== kind) {
                        Logger.system(`[p2p] ⇄ ${name || peerId.substring(0, 10)}… 连接方式切换: ${prev.path === 'relay' ? '中继' : '直连'} → ${kind === 'relay' ? '中继' : '直连 P2P'}`);
                    }
                    // 只更新当前连接仍是自己时（防新连接覆盖后又写回旧值）
                    if(prev?.conn === conn) {
                        this.onlinePeers.set(peerId, {path: kind, addr: selected.remoteAddr || '', conn});
                    }
                }
        ...
        Logger.system(`[p2p] ⇄ 节点已连接: ${peerId}${name ? `（${name}）` : ''} [${initInfo?.path === 'relay' ? '中继' : (initInfo?.path === 'p2p' ? '直连 P2P' : '检测中')}]（在线 ${this.onlinePeers.size}）`);

        // 清理死链：仅当关闭的还是当前登记的连接时才清（防新连接被旧连接的
        // closed 回调误删）
        conn.closed().then(() => {
            const cur = this.onlinePeers.get(peerId);
            if(cur?.conn === conn) {
    ...
    async handleBiStream(peerId, bi) {
        // send/recv 是 getter（d.ts: get send()）——连接已断的流上取属性可能抛
        let send, recv;
        try {
            send = bi.send;
            recv = bi.recv;
        } catch(e) {
            if(!this.isDisconnectError(e)) Logger.error('[p2p] 取流失败：' + (e?.message || e));
            return;
        }
        ...
        // iroh 调用安全包装：手机断网/流关闭时 writeAll/finish/reset/stop 会抛
        // ClosedStream 等 NAPI 错误。绝不能让错误经 stream 回调链逃逸成
        // uncaughtException（会击杀整个主服务）——统一转为日志 + shim 销毁。
        const safeCall = (fn, desc) => new Promise(resolve => {
            Promise.resolve().then(() => fn()).then(
                () => resolve(),
                e => {
                    if(!this.isDisconnectError(e)) {
                        Logger.error('[p2p] 流 ' + desc + ' 异常：' + (e?.message || e));
                    }
                    resolve();
                }
            );
        ...
        const shim = new Duplex({
            read() {},
            write(chunk, enc, cb) {
                // http 响应字节 → QUIC send 流；失败只销毁 shim，不上抛
                safeCall(() => send.writeAll(Array.from(chunk)), '写响应')
                    .then(() => cb());
            },
            final(cb) {
                safeCall(() => send.finish(), '结束发送').then(() => cb());
            },
            destroy(err, cb) {
                if(err) {
                    safeCall(() => send.reset(0n), 'reset');
                    safeCall(() => recv.stop(0n), 'stop');
                }
                cb();
            }
        });
        // shim 销毁兜底：http.Server 收到 socket 销毁会清理请求，无需响应
        shim.on('error', () => {});
    ...
    removePeer(peerId) {
        ...
        this.saveAllow();
        // 主动断开该节点的在线连接（否则已建立的 QUIC 连接继续可用）
        const online = this.onlinePeers.get(id);
        if(online?.conn) {
            try {
                online.conn.close(1008n, Array.from(Buffer.from('removed from allowlist')));
                Logger.system('[p2p] 已断开被移除节点的连接: ' + id);
            } catch(_) { /* 连接可能已关闭 */ }
        }
        return removed;
    },
    ...
    async shutdown() {
        if(!this.running) return;
        this.running = false;
        // ① 立即解绑主服务端口：后续 iroh 关闭可能慢/卡住（0~3s 兜底窗口），
        //    若端口等 process.exit 才释放，宝塔"停止→立即启动"的新进程会 bind 失败
        try {
            this.listenServer?.close();
            // Node 18.2+：顺带清掉存量 keep-alive 连接，防 close 回调挂着不返回
            this.listenServer?.closeAllConnections?.();
        } catch(_) {}
        // ② 停掉所有路径轮询定时器
